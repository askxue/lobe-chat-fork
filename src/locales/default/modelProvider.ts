export default {
  ...
  minimax: {
    ...
    unlock: {
      description:
        '输入你的 Minimax API Key 即可开始会话。应用不会记录你的 API Key',
  ...
  togetherai: {
    ...
    customModelName: {
      desc: '增加自定义模型，多个模型使用逗号（,）隔开',
      placeholder:
        'togethercomputer/Llama-2-7B-32K-Instruct,teknium/OpenHermes-2-Mistral-7B',
      title: '自定义模型名称'
    },
    title: 'TogetherAI',
    token: {
    ...
    unlock: {
      description:
        '输入你的 TogetherAI API Key 即可开始会话。应用不会记录你的 API Key',
  ...
  zeroone: {
    ...
    unlock: {
      description:
        '输入你的零一万物 API Key 即可开始会话。应用不会记录你的 API Key',
