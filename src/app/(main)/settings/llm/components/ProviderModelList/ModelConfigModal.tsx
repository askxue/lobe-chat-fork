  ...
  ({ showAzureDeployName, provider }) => {
    ...
    const [
      ...
      !!s.editingCustomCardModel &&
        provider === s.editingCustomCardModel?.provider,
    ...
    const modelCard = useUserStore(
      modelConfigSelectors.getCustomModelCard({
        id,
          ...
          >
            ...
            {showAzureDeployName && (
              <Form.Item
                ...
                label={t(
                  'llm.customModelCards.modelConfig.azureDeployName.title'
                )}
                name={'deploymentName'}
              >
                <Input
