...
const ModelIcon = memo<ModelProviderIconProps>(
  ({ model: originModel, size = 12 }) => {
    if (!originModel) {
      return;
    }
    ...
    if (model.includes('rwkv')) {
      return <Rwkv.Avatar size={size} />;
    }
    ...
    if (model.includes('hunyuan')) {
      return <Hunyuan.Avatar size={size} />;
    }
    // ref https://github.com/fishaudio/Bert-VITS2/blob/master/train_ms.py#L702
    if (
      model.startsWith('d_') ||
      model.startsWith('g_') ||
      model.startsWith('wd_')
    ) {
      return <FishAudio.Avatar size={size} />;
    }
    if (model.includes('skylark')) {
      return <ByteDance.Avatar size={size} />;
    }
    ...
    if (model.includes('firefly')) {
      return <Adobe.Avatar size={size} />;
    }
