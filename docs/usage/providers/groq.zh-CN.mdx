...
<Callout type={'info'}>
  Groq LPU 推理引擎在内部基准测试中连续达到每秒 300 个令牌的速度，据
  ArtificialAnalysis.ai 的基准测试确认，Groq 在吞吐量（每秒 241 个令牌）和接收
  100 个输出令牌的总时间（0.8 秒）方面优于其他提供商。
</Callout>
...
<Callout type={'warning'}>
  妥善保存弹窗中的 key，它只会出现一次，如果不小心丢失了，你需要重新创建一个
  key。
</Callout>
